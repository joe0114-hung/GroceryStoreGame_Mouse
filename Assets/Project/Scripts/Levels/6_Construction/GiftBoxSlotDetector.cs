...
public class GiftBoxSlotDetector : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // 自動改綁全域游標，避免抓到場景裡殘留的舊游標
        GameObject globalCursor = GameObject.Find("Cursor");
        if (globalCursor != null)
        {
            cursor = globalCursor.GetComponent<RectTransform>();
            Debug.Log("[GiftBoxSlotDetector] 🌟 已自動改綁全域游標！");
        }
        else
        {
            Debug.LogWarning("[GiftBoxSlotDetector] ⚠️ 找不到全域游標，將沿用 Inspector 綁定的舊值！");
        }
    }
